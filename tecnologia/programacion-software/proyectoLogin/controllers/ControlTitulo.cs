...
namespace proyectoLogin.Controllers
{
    public class ControlTitulo
    {
        ...
        {
            string comandoSQL =
            String.Format("exec GuardarTitulo '{0}'", fkSeccion);
        ...
        public string consultar(string id)
        {
            string retornado = "";
            string comandoSQL =
            String.Format("SELECT tblarticulo.fkidtitulo+'.'+tbltitulo.nombre FROM dbo.DetalleEvidencias " +
            ...
            "INNER JOIN dbo.tbltitulo ON dbo.tblarticulo.fkidtitulo = dbo.tbltitulo.id where evidencia.IdEvidencia='{0}'", id);
        ...
        {
            string comandoSQL =
            String.Format("exec ModificarTitulo '{0}', '{1}' ", id, capitulo);
            ControlConexion objControlConexion = new ControlConexion(BDatos);
            objControlConexion.abrirBD();
            objControlConexion.ejecutarComandoSQL(comandoSQL);
            objControlConexion.cerrarBD();
        ...
        {
            string comandoSQL = string.Format("SELECT * FROM TBLTITULO ORDER BY CAST(ID AS INT)");
            ControlConexion objControlConexion = new ControlConexion(BDatos);
            objControlConexion.abrirBD();
            DataSet objDataSet = objControlConexion.ejecutarConsultasSql(comandoSQL);
            objControlConexion.cerrarBD();

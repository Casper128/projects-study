    ...
    public class ControlArticulo
    {
       ...
       /* public void guardar(string fkCapitulo)
        ...
        public string consultar(string id)
        {
            string retornado = "";
            string comandoSQL =
            String.Format("SELECT tblarticulo.id+'.'+tblarticulo.nombre FROM dbo.DetalleEvidencias " +
        ...
        {
            string comandoSQL =
            String.Format("exec ModificarArticulo '{0}', '{1}' ", id, articulo);
            ControlConexion objControlConexion = new ControlConexion(BDatos);
            objControlConexion.abrirBD();
            objControlConexion.ejecutarComandoSQL(comandoSQL);
            objControlConexion.cerrarBD();
        ...
        {
            string comandoSQL = string.Format("SELECT * FROM TBLARTICULO ORDER BY CAST(ID AS INT)");
            ControlConexion objControlConexion = new ControlConexion(BDatos);
            objControlConexion.abrirBD();
            DataSet objDataSet = objControlConexion.ejecutarConsultasSql(comandoSQL);
            objControlConexion.cerrarBD();
